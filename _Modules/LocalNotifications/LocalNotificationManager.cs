...
{
    internal static class LocalNotificationManager
    {
        #region VARIABLES
        private static ILocalNotificationManager_Platform _platformManager;
        ...
        {
            #if UNITY_ANDROID
            _platformManager = new LocalNotificationManager_Android();
            #elif UNITY_IOS
        ...
        public static void ScheduleCustomNotifications(List<LocalNotification> _notifications)
        {
            foreach (var notification in _notifications)
            {
                ...
                // DEBUG
                LogMessage($"TYPE: Custom | Channel: {notification.Title} | Delivery Time: {DateTime.Now.Add(notification.FireTimeDelay).ToString_DDMMYYYYHHMMTT()}");
            }
        }
        #endregion
            ...
            foreach (var notificationInfo in filteredNotifications)
            {
                foreach (var txt in Database.freeResourcesTexts)
                {
                    if (txt.Identifier.Equals(notificationInfo.Identifier))
                    {
                        ...
                        // DEBUG
                        LogMessage($"TYPE: FreeResources | Data: {notificationInfo.Data} | Delivery Time: {DateTime.Now.Add(notification.FireTimeDelay).ToString_DDMMYYYYHHMMTT()}");
                        break;
                    }
                }
            }
            ...
            foreach (var notificationInfo in _notificationInfos)
            {
                foreach (var txt in Database.processesTexts)
                {
                    if (txt.Identifier.Equals(notificationInfo.Identifier))
                    {
                        ...
                        // DEBUG
                        LogMessage($"TYPE: Processes | Data: {notificationInfo.Data} | Delivery Time: {DateTime.Now.Add(notification.FireTimeDelay).ToString_DDMMYYYYHHMMTT()}");
